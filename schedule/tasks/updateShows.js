 ...
 * @returns {Promise<{deleted: true}|{deleted: false, poster: string, kinds: {id: string, name: string}[], duration: number, seasons: number, country: string, finished: boolean, nextEpisode: string, description: string?, creation: number?, network: string?, language: string?, episodes: number?}>}
 */
const fetchShowUpdate = async (show) => {
    ...
    const kinds = Object.entries(current.genres ?? {}).map(([id, name]) => ({id, name}));
