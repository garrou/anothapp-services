 ...
 * @returns {Promise<{deleted: true}|{deleted: false, poster: string, kinds: string, kindIds: string[], duration: number, seasons: number, country: string, finished: boolean, nextEpisode: string, description: string?, creation: number?, network: string?, language: string?, episodes: number?}>}
 */
const fetchShowUpdate = async (show) => {
    ...
    return {
        deleted: false, poster, kinds, kindIds, duration, seasons, country, finished, nextEpisode,
